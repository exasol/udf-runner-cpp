...
using exasol::udf::v2::third_party::nlohmann::json;
using exasol::udf::v2::third_party::nlohmann::json_patch;
using exasol::udf::v2::third_party::nlohmann::json_uri;
using exasol::udf::v2::third_party::nlohmann::json_schema::root_schema;
using namespace exasol::udf::v2::third_party::nlohmann::json_schema;
...
protected:
	virtual std::shared_ptr<schema> make_for_default_(
	    ...
	    root_schema * /* root */,
	    std::vector<exasol::udf::v2::third_party::nlohmann::json_uri> & /* uris */,
	    exasol::udf::v2::third_party::nlohmann::json & /* default_value */) const
	{
		return nullptr;
	};
...
protected:
	virtual std::shared_ptr<schema> make_for_default_(
	    ...
	    root_schema *root,
	    std::vector<exasol::udf::v2::third_party::nlohmann::json_uri> &uris,
	    exasol::udf::v2::third_party::nlohmann::json &default_value) const override
	{
...
public:
	logical_not(json &sch,
	            root_schema *root,
	            const std::vector<exasol::udf::v2::third_party::nlohmann::json_uri> &uris)
	    : schema(root)
	{
	...
	void validate(const json::json_pointer &ptr, const json &instance, json_patch &patch, error_handler &e) const final
	{
		...
		for (std::size_t index = 0; index < subschemata_.size(); ++index) {
			...
			if (!esub)
				...
				patch.get_json().get_ref<exasol::udf::v2::third_party::nlohmann::json::array_t &>().resize(oldPatchSize);
...
class type_schema : public schema
{
	...
	static std::shared_ptr<schema> make(json &schema,
	                                    json::value_t type,
	                                    root_schema *,
	                                    const std::vector<exasol::udf::v2::third_party::nlohmann::json_uri> &,
	                                    std::set<std::string> &);
	...
	void validate(const json::json_pointer &ptr, const json &instance, json_patch &patch, error_handler &e) const override final
	{
		...
		if (instance.is_null()) {
			patch.add(exasol::udf::v2::third_party::nlohmann::json::json_pointer{}, default_value_);
		}
	}
...
public:
	    ...
	    : schema(root), type_(static_cast<uint8_t>(json::value_t::discarded) + 1)
	{
		...
		// with exasol::udf::v2::third_party::nlohmann::json float instance (but number in schema-definition) can be seen as unsigned or integer -
		...
		// #54: JSON-schema does not differentiate between unsigned and signed integer - exasol::udf::v2::third_party::nlohmann::json does
...
public:
	    ...
	    : schema(root)
	{
		...
		if (attr != sch.end()) {
			...
			// special case for exasol::udf::v2::third_party::nlohmann::json-binary-types
			//
...
public:
	array(json &sch, root_schema *root, const std::vector<exasol::udf::v2::third_party::nlohmann::json_uri> &uris)
	    : schema(root)
	{
