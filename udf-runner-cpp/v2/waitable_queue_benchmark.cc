...
{
    auto queue =
        exasol::udf::v2::make_waitable_spsc_queue<int>(exasol::udf::v2::SpscQueue<int>(1024));
    for (const auto iteration : state)
    {
        benchmark::DoNotOptimize(&iteration);
        int value = 0;
...
void bm_waitable_spsc_batch(benchmark::State& state)
{
    ...
    const std::vector<int> batch(batch_size, 1);
    auto queue =
        exasol::udf::v2::make_waitable_spsc_queue<int>(exasol::udf::v2::SpscQueue<int>(batch_size));
...
void bm_waitable_spsc_epoll_latency(benchmark::State& state)
{
    ...
    for (const auto iteration : state)
    {
        ...
        benchmark_check(event.data.fd == queue.native_handle(), "unexpected epoll event");

        benchmark::DoNotOptimize(queue.drain_notifications());
        TimedItem item{};
        benchmark_check(queue.try_dequeue(item), "queue dequeue failed");
        benchmark_check(item.sequence == expected_sequence, "unexpected item sequence");
        ++expected_sequence;
