...
std::string read_string(const std::vector<char>& file, std::size_t offset, std::size_t maximum_size)
{
...
void verify_symbols(const std::string& library_path)
{
    ...
    const auto string_table =
        read_object<Elf64_Shdr>(file, section_table + dynamic_symbols.sh_link * sizeof(Elf64_Shdr));
...
int main(int argc, char** argv)
{
    try
    {
        ...
        const auto export_batch = reinterpret_cast<export_fn_t>(
            dlsym(handle, "udf_runner_cpp_v2_demo_export_record_batch"));
        const auto consume_batch = reinterpret_cast<consume_fn_t>(
            dlsym(handle, "udf_runner_cpp_v2_demo_consume_record_batch"));
        const auto last_error =
            reinterpret_cast<error_fn_t>(dlsym(handle, "udf_runner_cpp_v2_demo_last_error"));
        ...
        const auto batch = import_record_batch(&array, &schema);
