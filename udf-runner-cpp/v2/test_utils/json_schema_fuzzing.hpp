...
inline Json ReadSchema(const char *path) {
  ...
  if (!input.good()) {
    throw std::runtime_error(std::string("Unable to read JSON schema: ") +
                             path);
  }
...
public:
  explicit SchemaValidator(const char *root_schema_path)
      : validator_([](const isolated_nlohmann::json_uri &, Json &schema) {
          // The import and export schemas are the only schemas with an
          // external reference, and both reference this schema.
