...
{
public:
    explicit StartGate(const std::size_t participant_count) : participant_count(participant_count)
    {
    }

    void arrive_and_wait()
    {
        ready.fetch_add(1, std::memory_order::seq_cst);
        while (!go.load(std::memory_order::seq_cst))
        {
    ...
    void release()
    {
        while (ready.load(std::memory_order::seq_cst) != participant_count)
        {
            std::this_thread::yield();
        }
        go.store(true, std::memory_order::seq_cst);
    }
...
void produce_operation(Queue& queue,
                       const Byte operation_byte,
             ...
             std::atomic<bool>& producers_done)
{
    ...
    {
        produce_operation<Queue, Waitable>(queue, operation_byte, producer, sequence, produced);
               ...
               QueueFactory create_queue)
{
    ...
    {
        values.reserve(operations.size() * producer_count * (Waitable ? kMaxBatchSize : 1));
    }
    ...
    {
        producers.emplace_back(produce<Queue, Waitable>, std::ref(queue), std::ref(start_gate),
    ...
    {
        consumers.emplace_back(consume<Queue, Waitable>, std::ref(queue), std::ref(start_gate),
                               std::ref(consumed[consumer]), std::cref(producers_done));
    }
