...
#ifdef ENABLE_TEST_VM
#include "test_container/test_container.h"
#endif
...
std::function<SWIGVMContainers::SWIGVM*()> create_vm(const std::string& argv_lang, bool use_ctpg_options_parser) {
    ...
    else if(argv_lang.compare("lang=test") == 0) {
        #ifdef ENABLE_TEST_VM
            return []() { return new TestVM(false); };
        #else
            throw SWIGVMContainers::SWIGVM::exception("this exaudfclient has been compiled without test VM support");
        #endif
    }
    else {
